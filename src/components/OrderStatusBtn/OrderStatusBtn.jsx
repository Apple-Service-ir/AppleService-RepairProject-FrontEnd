...
function OrderStatusBtn(props) {
  const { status, children, clickHandler, isLoading } = props
  ...
  return (
    <button
      className={`${btnStyles} bg-white w-1/3 h-9 text-sm py-2 px-6 rounded-md
        flex justify-center items-center
        hover:text-white ${isLoading && 'cursor-default'}`}
